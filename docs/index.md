...
[GitHub](https://github.com/pmoust/audiorec){: .btn .mr-2 }
[Releases](https://github.com/pmoust/audiorec/releases){: .btn }
...
- **Crash-safe** — WAV headers are rewritten every 2s (configurable); a `kill -9` leaves playable files with at most one flush interval of tail loss.
...
- **Rolling segmentation** — `--segment-duration 30m` rotates tracks to `mic-001.wav`, `mic-002.wav`, ... All segments are independently playable.
- **Resampling** — `--sample-rate 48000` wraps all sources at a uniform rate before writing.
...
- **RF64 / large-file support** — WAV output transparently upgrades to RF64 when data exceeds 4 GB.
- **Session manifest** — `manifest.json` is written alongside track files with per-track format, timestamps, frame/byte/drop counters.
...
Pre-built binaries are also available on the [releases page](https://github.com/pmoust/audiorec/releases) — download the archive for your platform, verify the checksum, and put the binary on your `$PATH`. See the [user guide](user-guide#install) for the full download + verification steps.
...
| `--include-app IDS` | macOS only: comma-separated bundle IDs to include in system audio. |
| `--exclude-app IDS` | macOS only: comma-separated bundle IDs to exclude from system audio. |
...
| `--opus-bitrate VALUE` | Opus bitrate: integer or shorthand like `48k`. Default `48k`. |
...
macOS ties TCC permission grants to a signed bundle identifier. A plain `go build` binary is not a bundle, so each rebuild may force re-granting. For reliable permission persistence:

```bash
make app       # builds dist/audiorec.app with a stable bundle ID
...
**Opus on Linux** requires codec libraries installed before building:
